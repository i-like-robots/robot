 ...
 */
Robot.prototype.__readSource = function(callback) {
 ...
 * @return {Object}
 */
Robot.prototype.__templateData = function() {
  var data = {};
 ...
 * @param {Function} callback
 */
Robot.prototype.__generateOutput = function(callback) {
  var queue = new utils.Queue(callback);

  var globalData = this.__templateData();
  ...
  for (var page in this.pages.pages) {
    ...
    queue.wait();
    this.__saveToDisk(render, target, queue.each);
  }
};
 ...
 * @param {Function} callback
 */
Robot.prototype.__saveToDisk = function(data, target, callback) {
